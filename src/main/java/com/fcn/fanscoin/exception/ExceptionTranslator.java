...
import com.fcn.fanscoin.dto.v1.FieldErrorDTO;
...
public class ExceptionTranslator
    ...
    public ExceptionTranslator(final Environment env) {
        this.env = env;
    }
    ...
    public ResponseEntity<Problem> process(@Nullable final ResponseEntity<Problem> entity,
        ...
        ProblemBuilder builder = Problem
                .builder()
                .withType(Problem.DEFAULT_TYPE.equals(problem.getType()) ? ErrorConstants.DEFAULT_TYPE : problem
                        .getType())
    ...
    public ResponseEntity<Problem> handleMethodArgumentNotValid(final MethodArgumentNotValidException ex,
        ...
        List<FieldErrorDTO> fieldErrors = result
                .getFieldErrors()
                .stream()
                .map(f -> new FieldErrorDTO(
                        f.getObjectName().replaceFirst("DTO$", ""),
                        f.getField(),
                        StringUtils.isNotBlank(f.getDefaultMessage()) ? f.getDefaultMessage() : f
                                .getCode()))
    ...
    public ResponseEntity<Problem> handleEmailAlreadyUsedException(final EmailAlreadyUsedException ex,
        ...
        return create(problem,
                      request,
                      HeaderUtil.createFailureAlert(applicationName, false, problem.getEntityName(), problem
                              .getErrorKey(), problem.getMessage())
        );
    }

    @ExceptionHandler
    public ResponseEntity<Problem> handleUsernameAlreadyUsedException(final UsernameAlreadyUsedException ex,
                                                                      final NativeWebRequest request) {
    ...
    @ExceptionHandler
    public ResponseEntity<Problem> handleInvalidPasswordException(final InvalidPasswordException ex,
                                                                  final NativeWebRequest request) {
        return create(new InvalidPasswordException(), request);
    }

    @ExceptionHandler
    public ResponseEntity<Problem> handleBadRequestAlertException(final BadRequestAlertException ex,
                                                                  final NativeWebRequest request) {
        return create(ex,
                      request,
                      HeaderUtil.createFailureAlert(applicationName, false, ex.getEntityName(), ex.getErrorKey(), ex
                              .getMessage())
        );
    }
    ...
    private boolean containsPackageName(final String message) {
        return StringUtils
                .containsAny(message, "org.", "java.", "net.", "javax.", "com.", "io.", "de.", "com.fcn.fanscoin");
    }
}
